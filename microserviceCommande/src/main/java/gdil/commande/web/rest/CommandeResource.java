...
import gdil.commande.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link gdil.commande.domain.Commande}.
 */
@RestController
...
public class CommandeResource {
    ...
    private static final String ENTITY_NAME = "microserviceCommandeCommande";
    ...
    public ResponseEntity<CommandeDTO> createCommande(@Valid @RequestBody CommandeDTO commandeDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/commandes/" + commandeDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, commandeDTO.getId().toString()))
            .body(commandeDTO);
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<CommandeDTO>> getAllCommandes(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
    ...
    @GetMapping("/commandes-with-details")
    public List<CommandeWithDetailsDTO> getAllCommandesWithDetails() {
        return commandeService.findAllWithDetails();
    }
}
